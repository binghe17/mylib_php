...
/*
http://www.php.net/manual/zh/book.image.php
...
$image = imagecreatetruecolor($w*$row,$w*$line);
$c_text   = imagecolorallocate($image, 255,   255,   255);
for ($x=0; $x <$row ; $x++) { //x
	for ($y=0; $y <$line ; $y++) { //y
		$c_box   = imagecolorallocate($image, 30*$x,   30*$y,   0);//色块
		$c_line   = imagecolorallocate($image, 25*$x,   25*$y,   0);//色条
		$x1=$w*$y;
		$y1=$h*$x;
		$txt1=$x.','.$y;
		$txt2=$x1.','.$y1;
		$txt3=++$cc;
		imagefilledrectangle($image,$x1,$y1,$x1+$w,$y1+$h,$c_box);
		imagerectangle($image,$x1,$y1,$x1+$w,$y1+$h,$c_line);
		imagefttext($image,$font_size,0,$x1+5,$y1+$font_size+5,$c_text,'NanumGothic.ttf',$txt2);
		imagefttext($image,$font_size+30,0,$x1+25,$y1+$font_size+70,$c_text,'NanumGothic.ttf',$txt3);
	}
}
...
imagepng($image);
imagepng($image,'spritesheet3.png');//生成图片
